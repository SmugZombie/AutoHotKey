...
GetResults(){
	...
	SQL=sqlcmd -S ACCESSCONTROLSE\ECLIPSE -d DHS_MAIN -E -Q "SELECT Message.TimeStamp_Server,Message.Description,Message.MessageDescription,CardHolder.First_Name,CardHolder.Last_Name,Credential.Cardnumber FROM Message INNER JOIN CardHolder ON Message.CardHolderID=CardHolder.CardHolderID INNER JOIN Credential ON Message.CardHolderID=Credential.CardHolderID WHERE Message.TimeStamp_Server BETWEEN '%StartDate% 00:00:00' AND '%EndDate% 23:59:59' AND MessageDescription LIKE '`%`%Access`%`%'" -o "%output_file%" -W -w 999 -s","
	action := CMDRun(SQL)
